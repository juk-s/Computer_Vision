...
2. Если `lost == False`:
   ...
   * вычисление прямого и обратного оптического потока:
     ...
     * `p_back = calcOpticalFlowPyrLK(gray → prev_gray)`
     ...
     (в текущей версии ошибка вычисляется; отбор точек выполняется по флагам успешности `st`/`st2`).
   ...
   * при успехе:
     ...
     * обновление набора точек (оставляются inliers).
...
### 2.4. Параметры реализации (из кода)
...
Рекомендуемый формат фиксации результатов (заполнить по факту запуска):
...
* контролировать минимальное число матчей при re-detect и качество RANSAC (inliers/ошибка репроекции).
